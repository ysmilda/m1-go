...
// CallProcedure sends an RPC call to the target with the given header and procedure.
// It packs and unpacks the call and reply objects and checks for errors.
func CallProcedure[C any, R ReturnCoder](rw io.ReadWriter, header Header, procedure Procedure[C, R]) error {
	body, err := unpack.Pack(binary.LittleEndian, procedure.Call)
	if err != nil {
		return fmt.Errorf("unable to parse call: %w", err)
	}

	buf, err := Call(rw, header, body)
	if err != nil {
		return fmt.Errorf("unable to make rpc call: %w", err)
	}

	_, err = unpack.Unpack(buf, binary.LittleEndian, procedure.Reply)
	if err != nil {
		return fmt.Errorf("unable to parse reply: %w", err)
	}

	if err := m1errors.ParseReturnCode(procedure.Reply.GetReturnCode()); err != nil {
		return err
	}
...
func CallWithoutRead(rw io.ReadWriter, header Header, data []byte) error {
	body := []byte{}
	header.xID = uint32(rand.Int32()) // nolint:gosec
