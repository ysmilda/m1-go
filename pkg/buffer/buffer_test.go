...
func TestWriteStringTillLength(t *testing.T) {
	buf := NewBuffer(nil)
	buf.WriteStringTillLength("test", 10)
	check(t, "WriteStringTillLength", buf, "test\x00\x00\x00\x00\x00\x00")

	buf.Truncate(0)
	buf.WriteStringTillLength("test", 4)
	check(t, "WriteStringTillLength", buf, "test")

	buf.Truncate(0)
	buf.WriteStringTillLength("test", 1)
	check(t, "WriteStringTillLength", buf, "t")
}
...
func TestNewBufferShallow(t *testing.T) {
	...
	// Due to the implementation of the big/little endian reads, there will be a single allocation.
	if n > 1 {
		t.Errorf("allocations occurred while shallow copying: %f", n)
	}
	check(t, "NewBuffer", &buf, testString)
}
...
var readBytesTests = []struct {
	...
	{"", 0, []string{""}, nil},
	{"a\x00", 0, []string{""}, nil},
	{"abbbaaaba", 2, []string{"ab", "bb", "aa", "ab", "a"}, io.EOF},
	{"hello\x01world", 10, []string{"hello\x01worl", "d"}, io.EOF},
}
...
func TestReadBytes(t *testing.T) {
	for _, test := range readBytesTests {
		...
		for _, expected := range test.expected {
			var bytes []byte
			bytes, err = buf.ReadBytes(test.n)
...
func TestGrowOverflow(t *testing.T) {
	defer func() {
		if err := recover(); !errors.Is(err.(error), ErrTooLarge) {
