...
type Target struct {
	...
	// VHD - The visualization handler acts as a communication interface between visualization
	// clients and M1 software modules. To use this module one must be logged in and have
	// initialized the VHD module by calling target.InitializeVHD().
	VHD *VhdModule

	// INF - The system information handler collects and condenses information from all over the M1 controller.
	INFO *InfoModule
...
func NewTarget(ip net.IP, protocol string, timeout time.Duration) (*Target, error) {
	client := client.NewClient(ip, timeout, protocol)
