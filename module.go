...
func newModule(client *client.Client, name string, info res.ModuleNumber, msysVersion msys.Version) (*Module, error) {
	m := &Module{
		client:      client,
		name:        name,
		info:        info,
	...
	err := client.AddConnection(info.ModuleNumber, info.UDPPort, info.TCPPort)
	if err != nil {
...
// 		returnCode, _ := buf.LittleEndian.ReadUint32()
// 		if returnCode == (msys.SourceSVI | msys.ErrorFAILED) {
// 			// This is how the system announces that there are no more variables.
// 			break
// 		} else if err := msys.ParseReturnCode(returnCode); err != nil {
// 			return nil, err
// 		}

// 		count, _ := buf.LittleEndian.ReadUint32()
// 		for range count {
// 			name, _ := buf.ReadString(svi.NameLength)
// 			format, _ := buf.LittleEndian.ReadUint16()
// 			length, _ := buf.LittleEndian.ReadUint16()
